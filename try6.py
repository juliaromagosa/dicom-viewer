                 ...
                 children=[
                     ...
                     dbc.CardFooter(
                         [
                             dbc.Row([
                                     dbc.Col([
                                         dbc.Button("Previous", id='previous_button3', color="primary", className="mr-1"),
                                         dbc.Button('Next', id='next_button3', color="primary", className='mr-1'),
                                     ]),
                                     dbc.Col(html.H6(
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Store(id='memory'),dcc.Store(id="memory2"),dcc.Store(id="memory3"),
                            dcc.Store(
                                id="image_files",
                                data={"files":filelist_ADC,"current": 0},
                            ),
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Store(
                                id="image2_files",
                                data={"files2": filelist_3DT2, "current2":0},
                            ),
                        ],
                    ),
                ),
                        ...
                        [
                            ...
                            dcc.Store(
                                ...
                                data=dict(
                                    **{
                                        filename3: {"shapes": []}
                                        for filename3 in filelist_Perfusion
                                    },
                                    **{"starttime": time_passed()}
                                ),
                            ),
                            dcc.Store(
                                id="image3_files",
                                data={"files3": my_dict, "space":1 ,"time":0},
                            ),
                        ],
                    ),
                ),
...
@app.callback(
    [Output("memory","data"),Output("memory2","data"),Output("memory3","data")],
    [Input("tittle_card_1_output","children")],

)
def update_filelist(patients):

    filelist_ADC,filelist_3DT2, filelist_Perfusion=create_filelist(patients)
    return filelist_ADC,filelist_3DT2, filelist_Perfusion
...
def update_output_1(filename,image_files_data):
    filename = image_files_data["files"][image_files_data["current"]]
    return ['Image {}/{}'.format(pydicom.dcmread(filename).InstanceNumber,Images_ADC)]
...
def update_output_2(filename2, image2_files_data):
    filename2 = image2_files_data["files2"][image2_files_data["current2"]]
    return ['Image: {}/{}'.format(pydicom.dcmread(filename2).InstanceNumber,Images_3DT2)]
...
def modify_table_entries(
    ...
    image_files_data["current2"] %= len(image_files_data["files2"])
    if image_index_change != 0:
        # image changed, update annotations_table_data with new data
        annotations_table_data = []
        filename = image_files_data["files2"][image_files_data["current2"]]
        debug_print(annotations_store_data[filename])
        for sh in annotations_store_data[filename]["shapes"]:
            annotations_table_data.append(shape_to_table_row(sh))
        return (annotations_table_data, image_files_data)
...
def send_figure_to_graph(
    ...
    if annotations_table_data is not None:
        filename2 = image_files_data["files2"][image_files_data["current2"]]
        # convert table rows to those understood by fig.update_layout
        fig_shapes = [table_row_to_shape(sh) for sh in annotations_table_data]
        debug_print("fig_shapes:", fig_shapes)
        debug_print(
...
def modify_table_entries(
    ...
    image_files_data["current3"] %= len(image_files_data["files3"])
    if image_index_change != 0:
        # image changed, update annotations_table_data with new data
        annotations_table_data = []
        filename = image_files_data["files2"][image_files_data["current2"]]
...
@app.callback(
    ...
    [State("image3_files", "data"), State("annotations3-store", "data")],
)
def send_figure_to_graph(
    annotations_table_data, annotation_type, image_files_data, annotations_store
):
    if annotations_table_data is not None:
        filename3 = image_files_data["files2"][image_files_data["current2"]]
        # convert table rows to those understood by fig.update_layout
        fig_shapes = [table_row_to_shape(sh) for sh in annotations_table_data]
        debug_print("fig_shapes:", fig_shapes)
        debug_print(

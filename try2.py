...
methods = ["3DT2", "ADC", "Perfusion"]
methods_default = methods[0]
methods_dictionary = dictionary_function(methods)
...
image_id_method_before = -1
image_id_image_before = -1
...
image_annotation_card = dbc.Card(
    ...
    children=[
        ...
        dbc.CardFooter(
            [
                dcc.Markdown(
                    "** Select patient and method to choose an image ** \n\n"
                ),
                dbc.Row([
                    dbc.Col(
                        dcc.Dropdown(
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Store(
                                id="image_files",
                                data={"files": filelist, "current_patient": 0, "current_method":0, "current_image":0},
                            ),
                        ],
                    ),
                ),
...
@app.callback(
    ...
    [
        ...
        Input(component_id="method_button", component_property="value"),
        Input(component_id="image_button", component_property="value"),

    ]
)
#NUEVO
@app.callback(
    Output('image_button','options'),
    Input('method_button','value')
)
def set_images_options(selected_method):
    return [{'label': i, 'value': i} for i in image_dictionary[selected_method]]

@app.callback(
    Output("image","value"),
    Input('image_button', 'options')
)
def image_output_value(availables_options):
    return availables_options[0]['value']

#FIN NUEVO

def update_output_div(patient, method):
    return '{}, method {} '.format(patient, method)
...
def modify_table_entries(
    ...
    global image_id_patient_before, image_id_method_before, image_id_image_before
    global patient_dictionary, methods_dictionary, image_dictionary
    ...
    image_id_method = methods_dictionary[id_method]
    image_id_image=image_dictionary[id_image]
    ...
    image_files_data["current_method"] = image_id_method
    image_files_data["current_image"] = image_id_image
    #image_files_data["current"] %= len(image_files_data["files"])

    if (image_id_patient != image_id_patient_before or image_id_method != image_id_patient_before):
        image_id_patient_before = image_files_data["current_patient"]
        image_id_method_before = image_files_data["current_method"]
        image_id_image_before = image_files_data["current_image"]

        # image changed, update annotations_table_data with new data
        annotations_table_data = []
        filename = image_files_data["files"][image_files_data["current_patient"]][image_files_data["current_method"]][image_files_data["current_image"]]
...
@app.callback(
    ...
    Input("patient_button", "value"), Input("method_button", "value"), Input("image_button", "value")],
    [State("image_files", "data"), State("annotations-store", "data")],
)
def send_figure_to_graph(
    annotations_table_data, annotation_type, num_patient, num_method, num_image, image_files_data, annotations_store
):
